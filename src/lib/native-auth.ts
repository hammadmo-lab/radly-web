...
export async function signInWithAppleNative() {
  try {
    ...
    // The plugin may have renamed or nested it differently
    let idToken: string | undefined

    // Path 1: Direct idToken property
    if (authData.idToken && typeof authData.idToken === 'string') {
      idToken = authData.idToken
      console.log('🍎 Found idToken at authData.idToken')
    }
    // Path 2: Wrapped in accessToken object
    else if (authData.accessToken?.idToken && typeof authData.accessToken.idToken === 'string') {
